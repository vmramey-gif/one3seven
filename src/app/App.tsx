...
import type { FirmDashboardRow, FirmLiveIntakeView } from '../services/firmRoutingService';
import type { PersistentNotificationRow } from '../services/workerActivityService';
import type {
  ProfileRow,
  FirmProfileRow,
 ...
 * 'landing' ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â their draft is saved server-side, so no work is lost.
 */
...
export default function App() {
  const [currentScreen, setCurrentScreen] = useState<Screen>(() => {
    ...
    try {
      ...
      // Lands the worker on the low-friction Create Account (or Sign in) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â not the firm homepage.
  ...
  // The firm's OWN case files (documents the firm uploaded and organized itself) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â kept separate
  ...
  // (not because there was simply no route metadata to load from) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â distinguishes "we tried to
  ...
  const [workerLiveSummary, setWorkerLiveSummary] = useState<{
    ...
    readiness: string[];
    missing: string[];
    /** Dated payroll/wage records (raw strings) for Gap Detection ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â from the pre-collapse timeline. */
    payrollDates: string[];
    /** Employer name confirmed from document extraction, when the worker hasn't stated one. */
    confirmedEmployer: string | null;
    /** Employment period ("start ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ end") confirmed from document extraction. */
  ...
  const [workerIntakeRoutingCards, setWorkerIntakeRoutingCards] = useState<
    firmRouting.WorkerIntakeFirmRoutingCard[]
  >([]);
  ...
  /** Upload-screen ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“Add ContextÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â; flushed into Supabase intake_summaries after org persist, or into workspace when offline. */
  ...
  /** Any saved intake row means the worker already started organizing ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â skip the first-run law-firm modal. */
  useEffect(() => {
    if (!profile?.id || profile.role !== 'worker') return;
  ...
  function routeSafeAfterProfileBootstrapFailure(signedInUser: User, reason: string) {
    try {
      console.error('[o3s-post-auth] profile bootstrap failed ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â safe route', {
        reason,
      ...
      // load here, we have no verified role for this user ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â client-only signals (firmIntent,
      ...
      if (meta === 'worker') {
        setUserRole('worker');
        firmSignInIntentRef.current = false;
        console.info('[o3s-post-auth] safe route ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ landing', { meta });
        setCurrentScreen('landing');
        return;
      }
      setUserRole(null);
      firmSignInIntentRef.current = false;
      console.info('[o3s-post-auth] safe route ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ roleSelection');
  ...
  async function refreshFirmDashboardRows(fp: FirmProfileRow) {
    ...
    const { rows, error: dashError } = await firmRouting.loadFirmDashboardRows(fp.id);
    ...
    if (authUser?.id) {
      try {
        setFirmOwnCaseFiles(await firmRouting.listWorkerIntakes(authUser.id));
      } catch (e) {
  ...
  async function refreshWorkerSummaryLive(intakeId: string) {
    ...
    const confirmedEmploymentPeriod = start && end ? `${start} ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ ${end}` : start || end || null;
  ...
  async function refreshWorkerAccessRows(forUserId?: string) {
    ...
    if (!id) return;
    const rows = await firmRouting.listWorkerAccessRequests(id);
  ...
  useEffect(() => {
    const { data: sub } = supabase.auth.onAuthStateChange((evt, session) => {
      ...
      // A password-recovery link lands here with a real (recovery-scoped) session ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â route
      ...
      const runPostAuth = () => {
        ...
        void (async () => {
          try {
            ...
            const { profile: p, error: profileEnsureError } = await authProfile.ensureUserProfile(
              signedInUser,
            ...
            try {
              ...
              if (metaFullName && !((p.full_name ?? '').trim())) {
                const nameRes = await authProfile.updateProfileName(signedInUser.id, metaFullName);
              ...
              if (signupPhone.trim()) {
                const phoneRes = await authProfile.saveWorkerContactDetails(signedInUser.id, {
            ...
            if (currentScreenRef.current === 'roleSelection') {
              console.info('[o3s-post-auth] on roleSelection ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â skip auto-routing until user commits role');
              return;
            }
              ...
              const display =
                typeof metaGoogle.full_name === 'string'
                  ...
                  : typeof metaGoogle.name === 'string'
                    ...
                    : '';
              const pr2 = await authProfile.fetchProfile(signedInUser.id);
              if (display && pr2 && !((pr2.full_name ?? '').trim())) {
                const unp = await authProfile.updateProfileName(signedInUser.id, display);
                if (!unp.error) {
                  const pr3 = await authProfile.fetchProfile(signedInUser.id);
                  setProfile(pr3);
                  routingProfile = pr3;
                }
              }
            ...
            if (routingProfile?.role === 'firm') {
              setUserRole('firm');
              firmProfileForRouting = await authProfile.ensureFirmProfile(
                signedInUser.id,
                signedInUser.email ?? null
              );
              setFirmProfile(firmProfileForRouting);
              if (firmProfileForRouting && authProfile.isFirmProfileComplete(firmProfileForRouting)) {
            ...
            if (routingProfile?.role === 'worker') {
              ...
              if (fdRaw) {
                try {
                  ...
                  if (fullName) await authProfile.updateProfileName(signedInUser.id, fullName);
                  if (fdData.phone) await authProfile.saveWorkerContactDetails(signedInUser.id, { phone: fdData.phone });
            ...
            if (!handledFirmDirectedReturn && AUTH_FLOW_SCREENS.includes(screen)) {
              ...
              if (routingProfile?.role === 'firm') {
                const firmScreen = authProfile.firmProfileNeedsSetup(firmProfileForRouting)
                  ? 'firmSettings'
                  : 'firmDashboard';
                firmSignInIntentRef.current = false;
                ...
                // deep-restored, and only if that intake still exists; everything else ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ case list.
  ...
  useEffect(() => {
    const onVisible = () => {
      ...
      console.info('[o3s-post-auth] visibility retry ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â scheduling deferred post-auth', { screen });
      const runRetry = () => {
        if (postAuthInFlightRef.current) return;
        postAuthInFlightRef.current = true;
        void (async () => {
          try {
            const { profile: p, error: profileEnsureError } = await authProfile.ensureUserProfile(signedInUser);
            if (profileEnsureError || !p) {
            ...
            if (p.role === 'firm') {
              setUserRole('firm');
              const fp = await authProfile.fetchFirmProfileForUserWithTimeout(signedInUser.id);
              setFirmProfile(fp);
              if (AUTH_FLOW_SCREENS.includes(currentScreenRef.current)) {
                setCurrentScreen(authProfile.firmProfileNeedsSetup(fp) ? 'firmSettings' : 'firmDashboard');
              }
  ...
  useEffect(() => {
    ...
    void firmRouting.updateIntakeWorkflowStatus(currentIntakeId, 'Organizing Records');
  }, [currentScreen, currentIntakeId]);
  ...
  useEffect(() => {
    ...
    void firmRouting.updateIntakeWorkflowStatus(currentIntakeId, 'Awaiting Worker Approval');
  ...
  const commitPendingWorkerIntake = useCallback(async (): Promise<string | null> => {
    ...
    // finishes ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â share one in-flight creation so we never insert two intakes/numbers.
  ...
  const categoryFromGuidedAnswers = useCallback(
    (intakeId: string): IntakeCaseCategory | typeof BETA_WORKER_CASE_CATEGORY | null => {
      ...
      if (fromLocal) {
        ...
        // unreachable ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â fromLocal's type (IntakeCaseCategory) never actually contains the
  ...
  const completeGuidedIntakeFlow = useCallback(
    async (answers: GuidedIntakeAnswers) => {
      ...
      console.info('[o3s-intake] completeGuidedIntakeFlow ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ upload', {
  ...
  const skipGuidedIntakeFlow = useCallback(() => {
    ...
    clearWorkerIntakeUiState();
    console.info('[o3s-intake] skipGuidedIntakeFlow ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ upload', {
      ...
      if (intakeId) {
        ...
        if (isSupabaseConfigured()) {
          void (async () => {
            ...
            // (no-op if no summary row exists yet ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â safe to always call)
  ...
  // Firm-directed intake ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â routes directly to the intake form, no login required upfront.
  ...
  const handlePersistentNotificationClick = useCallback(
    async (row: PersistentNotificationRow) => {
      const markResult = await workerActivity.markNotificationRead(row.id);
      if (markResult.error) {
      ...
      if (
        ...
        if (meta) {
          ...
          try {
            const v = await firmRouting.loadFirmLiveIntakeView(
              intakeId,
              meta.routeId,
              routeStatus,
              meta.intakeNumber
  ...
  const refreshPersistentNotifications = useCallback(async () => {
    ...
    const { rows, error } = await workerActivity.listNotificationsForUser();
    if (error) {
  ...
  useEffect(() => {
    if (!isSupabaseConfigured() || !isAuthenticated || !authUser?.id) return;
    const unsubscribe = workerActivity.subscribeToOwnNotifications(() => {
      void refreshPersistentNotifications();
    });
    return unsubscribe;
  ...
  const navigateToScreen = (screen: Screen) => {
    ...
    if (isFirmUser && screen === 'firmDashboard' && authProfile.firmProfileNeedsSetup(firmProfile)) {
  ...
  useEffect(() => {
    const isFirmUser = userRole === 'firm' || profile?.role === 'firm';
    if (!isFirmUser || !authProfile.firmProfileNeedsSetup(firmProfile)) return;
  ...
  const refreshWorkerIntakesList = async (userId?: string): Promise<WorkerIntakeListEntry[]> => {
    ...
    try {
      const rows = await firmRouting.listWorkerIntakes(id);
      setWorkerIntakesList(rows);
      const cards = await firmRouting.listWorkerIntakeFirmRoutingCards(id);
  ...
  const handleRemoveFirmCodeForIntake = async (intakeId: string) => {
    ...
    const result = await firmRouting.removeFirmCodeFromIntake(intakeId);
    if (result.error) {
  ...
  const handleDeleteWorkerIntake = async (intakeId: string) => {
    ...
    const result = await firmRouting.deleteWorkerOwnedIntake(intakeId);
    if (result.error) {
    ...
    // Intake rows are deleted, but storage cleanup could not be confirmed ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â tell the worker
  ...
  const handleGoogleSignIn = async () => {
    ...
    if (error) window.alert(/not enabled|unsupported provider/i.test(error.message) ? 'Google sign-in isnÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢t available yet ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â please continue with email and password.' : error.message);
  };
  ...
  const linkFirmCodeForActiveIntake = async (firmId: string) => {
    ...
    const link = await firmRouting.linkFirmCodeToIntake(intakeId, firmId);
    if (link.error) {
      return { error: toBetaUserMessage(link.error, 'Could not link this firm code. Try again.') };
    }
    setWorkerIntakeChannel('firm_code');
    const display = await firmRouting.fetchWorkerIntakeRoutingDisplay(intakeId);
  ...
  const prelinkFirmCodeForActiveIntake = async (
    ...
    const intakeId = currentIntakeIdRef.current;
    if (!intakeId) return { error: 'No active intake.' };
    const firm = await firmRouting.fetchFirmByCodeForWorker(code);
    if (!firm) return { error: 'Firm code not found. Check the code and try again.' };
    const link = await firmRouting.linkFirmCodeToIntake(intakeId, firm.id);
    if (link.error) {
      return { error: toBetaUserMessage(link.error, 'Could not link this firm code. Try again.') };
    }
    setWorkerIntakeChannel('firm_code');
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!result.error) {
        try {
          sessionStorage.removeItem('o3s_prefill_fc');
          sessionStorage.removeItem('o3s_firm_ctx'); // intake is linked ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â context no longer needed
        } catch { /* ignore */ }
  ...
  // Firm-directed entry ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â resolve ?fc= firm code into context on mount (once).
  // If o3s_firm_ctx is already in sessionStorage (restored above in useState init),
  // skip the async lookup ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â context is already set. Otherwise resolve from o3s_prefill_fc.
  useEffect(() => {
    if (!FIRM_CODE_ROUTING_LIVE) return;
    if (!isSupabaseConfigured()) return;
    // Already restored synchronously from sessionStorage ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â nothing to do.
    ...
    void (async () => {
      ...
      try {
        firm = await firmRouting.fetchFirmByCodeForWorker(trimmed);
      } catch (e) {
      ...
      if (firm?.id && firm.firm_name) {
        ...
        // Unresolvable or errored firm code ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â never leave the visitor on the infinite
  ...
  // Stripe billing return ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â surface notification when firm returns from checkout or portal
  useEffect(() => {
    if (profile?.role !== 'firm') return;
  ...
  const persistNewFiles = async (files: File[]) => {
    ...
    // Collect failures instead of alerting per-file ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â a batch of N failures used to stack N
    ...
    try {
      ...
      if (failedCount > 0) {
        const detail = failedFiles.map((f) => `ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã¢â‚¬Å“${f.name}ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â: ${f.message}`).join('; ');
        throw new Error(
          `${failedCount} file${failedCount === 1 ? '' : 's'} could not be uploaded ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ${detail}`
        );
      }
      if (duplicateCount > 0) {
  ...
  const handleConfirmDocumentRequestResponse = async (payload: {
    ...
    if (
      ...
      return {
        error:
          'Your response did not save completely. The request is still active ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â select categories and try again.',
      };
    }
  ...
  > => {
    ...
    const hasSummaryRow = await firmRouting.workerIntakeHasSummaryRow(intakeId);
    if (!hasSummaryRow) {
      return {
        ok: false,
        error:
    ...
    await refreshWorkerSummaryLive(intakeId);
    void firmRouting.ensureLinkedFirmPreviewRoute(intakeId);
  ...
  const handleReorganizeIntake = useCallback(async () => {
    ...
    // Without this, "Re-organize file" only re-summarized whatever extraction had already stored ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â
    ...
    if (err.error) {
      ...
      pushWorkerNotification({
        ...
        body: 'We could not rebuild your summary just now. Your existing summary is unchanged ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â try again in a moment.',
      });
    }
    if (!err.error) {
      ...
      // organize ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â otherwise the firm packet header falls back to "Not yet identified".
      const routing = await firmRouting.fetchWorkerIntakeRoutingDisplay(intakeId);
      if (routing.submissionChannel === 'firm_code' || routing.linkedFirmId) {
        const contactErr = await mergeWorkerContactIntoLatestIntakeSummary(intakeId, {
          name: (profile?.full_name ?? '').trim() || null,
  ...
  const shareParticipating = async () => {
    ...
    const r = await firmRouting.routePreviewToParticipatingFirms(currentIntakeId);
    if (r.error) {
      return {
        error: toBetaUserMessage(
          r.error,
    ...
    await firmRouting.markIntakeSubmitted(currentIntakeId, { workflow_status: 'Matching Participating Firms' });
    if (profile?.id) {
  ...
  const shareFirmCode = async (code: string) => {
    ...
    const routing = await firmRouting.fetchWorkerIntakeRoutingDisplay(currentIntakeId);
    const r =
      routing.submissionChannel === 'firm_code' && routing.linkedFirmId
        ? await firmRouting.sendOrganizedIntakeToLinkedFirm(currentIntakeId)
        : await firmRouting.routeIntakeToFirmCode(currentIntakeId, code);
    ...
    // Worker-facing confirmation ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â the manual share previously only notified the FIRM,
  ...
  const approveWorkerAccess = async (routeId: string) => {
    ...
    if (!res.error) {
      // Approving full access is a consent moment too ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â make sure the firm now sees the contact.
      if (currentIntakeId) {
        const contactErr = await mergeWorkerContactIntoLatestIntakeSummary(currentIntakeId, {
          name: (profile.full_name ?? '').trim() || null,
          phone: (profile.phone ?? '').trim() || null,
  ...
  const declineWorkerAccess = async (routeId: string) => {
    if (!profile?.id) return { error: 'Not signed in' };
    const res = await firmRouting.workerDeclineFullAccess(routeId, profile.id);
    if (!res.error) {
      await refreshWorkerAccessRows();
      if (currentIntakeId) await refreshWorkerSummaryLive(currentIntakeId);
      void refreshPersistentNotifications();
  ...
  // (detectSessionInUrl, on by default) fires a PASSWORD_RECOVERY auth event automatically ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â
  ...
  const handleCreateAccount = async (
    ...
    if (!data.session?.user) {
      console.info(
        '[o3s-auth-audit] signUp: no session returned (common when "Confirm email" is enabled). Check Auth ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ Users and user email inbox.',
        { userId: data?.user?.id ?? null }
      );
    ...
    if (data.session?.user) {
      ...
      setIsAuthenticated(true);
      setCurrentScreen('roleSelection');
      // Profile load/create runs in deferred onAuthStateChange ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â do not await Supabase here.
    }
  ...
  const handleWorkerGoogleSignUpCreate = async () => {
    ...
    if (error) {
      ...
      window.alert(/not enabled|unsupported provider/i.test(error.message) ? 'Google sign-in isnÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â‚¬Å¾Ã‚Â¢t available yet ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â please continue with email and password.' : error.message);
    }
  };
  ...
  const handleWorkerDetailsComplete = async (details: WorkerDetailsPayload) => {
    ...
    const r = await authProfile.updateProfileName(authUser.id, fullName);
    if (r.error) return { error: r.error };
    // Save contact details to profiles table (persistent, device-independent)
    const contactSave = await authProfile.saveWorkerContactDetails(authUser.id, {
    ...
    const pr = await authProfile.fetchProfile(authUser.id);
    setProfile(pr);
    setCurrentScreen('landing');
    return {};
  };
  ...
  const handleCommitRole = async (role: 'worker' | 'firm') => {
    ...
    authProfile.resetEnsureUserProfileInflight();
    console.info('[o3s-role-commit] before commitProfileRoleForUser', { role });
    const commit = await authProfile.commitProfileRoleForUser(authUser, role);
    ...
    const p = commit.profile ?? authProfile.profileRowFromAuthUser(authUser, role);
    setProfile(p);
    setUserRole(role);
    try {
      sessionStorage.removeItem(O3S_SS_WORKER_PENDING_DETAILS);
    ...
    if (role === 'firm') {
      let fp: authProfile.FirmProfileRow | null = null;
      if (!commit.timedOut) {
        console.info('[o3s-role-commit] before fetchFirmProfileForUserWithTimeout', { role });
        fp = await authProfile.fetchFirmProfileForUserWithTimeout(authUser.id);
      } else {
      ...
      console.info('[o3s-role-commit] after firm profile resolve', {
        ...
        needsSetup: authProfile.firmProfileNeedsSetup(fp),
      });
      if (fp && authProfile.isFirmProfileComplete(fp)) {
        try {
      ...
      const firmNext = authProfile.firmProfileNeedsSetup(fp) ? 'firmSettings' : 'firmDashboard';
  ...
  // is visible during testing ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â replaced with in-app UI in a later increment.
  ...
  const handleFirmOrganize = async () => {
    ...
    try {
      ...
      // persistNewFiles no longer alerts per-file (H13 fix) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â without this catch, a failure
  ...
  const handleFirmRequestAdditionalDocuments = async (payload: {
    ...
    const result = await firmRouting.firmRequestAdditionalDocuments(
      selectedRouteMeta.routeId,
      categories,
  ...
  const handleFirmAddWorkerReminder = async (payload: {
    ...
    const result = await firmRouting.firmAddWorkerReminder(
      selectedRouteMeta.routeId,
      payload.text,
      payload.dueDate
    );
  ...
  const openFirmSampleIntakeFlow = () => {
    ...
    const demoView: FirmLiveIntakeView = {
      ...
      events: [
        {
          id: 'sample-ev-1',
          event_date: 'Jan ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã…â€œ Mar 2026',
  ...
  // Canonical "brand logo ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ home" rule for every corner wordmark:
  //   ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¢ logged-in worker ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ their case-list dashboard (via handleLogoClick, which preserves the
  //     unsaved-progress exit modal)
  //   ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¢ logged-in firm   ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ the firm dashboard
  //   ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¢ everyone else (logged out / marketing) ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ the worker home landing page (forWorkers)
  ...
  const workerHubIntakeLabel = workerOnboardingPrePersist
    ...
    : currentIntakeId
      ...
      : workerLandingIntakeRow
        ...
        : hasCompletedIntake
          ? `Local ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â· ${currentIntakeWorkspace.id.replace(/^intake-/, '').slice(0, 12)}`
          : null;
  ...
  // Worker Home "at a glance" data ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â derived from the already-loaded live summary so the Home hub
  ...
  // file" modal popped on the very first "Start organizing" ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â a firm choice before the win.)
    ...
    return (
      <div className="min-h-screen bg-[#F1F3EF] flex items-center justify-center px-6">
        <div className="text-center">
          ...
          <p className="mt-2 text-sm text-[#6A6D66]">Restoring your secure sessionÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦</p>
        </div>
      </div>
    );
  }

  // ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ Beta access gate ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã‚ÂÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬
  ...
  // product itself. Approve an account: update public.profiles set approved = true where email = 'ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦'.
  const accountGated =
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white">
      {OFFLINE_DEV_GALLERY_ONLY ? (
        <div
          ...
          role="status"
        >
          Dev UI gallery only ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬ÃƒÂ¢Ã¢â€šÂ¬Ã‚Â Supabase is not configured. Set{' '}
        ...
        <AnimatePresence mode="wait">
          ...
          {currentScreen === 'firmDirectedIntake' && !firmDirectedContext && (
            <motion.div key="firmDirectedIntakeLoading" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <span className="text-sm text-[#1B2623]/40">LoadingÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€šÃ‚Â¦</span>
            </motion.div>
          )}
          {currentScreen === 'firmDirectedIntake' && firmDirectedContext && (
            <motion.div
          ...
          {currentScreen === 'upload' && (
            ...
            >
              <UploadScreen
                ...
                onLookupFirmCode={isSupabaseConfigured() ? (code) => firmRouting.fetchFirmByCodeForWorker(code) : undefined}
                onLinkFirmToIntake={
          ...
          {currentScreen === 'firmDashboard' && (
            ...
            >
              <LawFirmDashboardScreen
                ...
                firmBanner={
                  firmProfile && authProfile.isFirmProfileComplete(firmProfile)
                    ? {
              ...
              <IntakeReviewScreen
                ...
                onRequestFullAccess={
                  selectedRouteMeta && isSupabaseConfigured() && !firmLiveView?.isFirmCodeIntake
                    ? async () => {
                        const result = await firmRouting.firmRequestFullAccess(selectedRouteMeta.routeId);
                        if (!result.error) {
                ...
                onAcceptIntake={
                  selectedRouteMeta && isSupabaseConfigured()
                    ? async () => {
                        const result = await firmRouting.firmAcceptIntake(selectedRouteMeta.routeId);
                        if (!result.error) {
                          await reloadFirmLiveViewForSelection();
                          if (firmProfile) void refreshFirmDashboardRows(firmProfile);
                        }
                ...
                onDeclineIntake={
                  selectedRouteMeta && isSupabaseConfigured()
                    ? async () => {
                        const result = await firmRouting.firmDeclineIntake(selectedRouteMeta.routeId);
                        if (!result.error) {
                          await reloadFirmLiveViewForSelection();
                          if (firmProfile) void refreshFirmDashboardRows(firmProfile);
                        }
          ...
          {currentScreen === 'firmSettings' && (
            ...
            >
              <FirmSettingsScreen
                ...
                setupRequired={authProfile.firmProfileNeedsSetup(firmProfile)}
                onFirmProfileUpdated={(fp) => {
                  setFirmProfile(fp);
                  if (authProfile.isFirmProfileComplete(fp)) void refreshFirmDashboardRows(fp);
                }}
