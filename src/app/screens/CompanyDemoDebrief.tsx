...
const field =
  'w-full rounded-[11px] border border-[#C6D0C8] bg-white px-4 py-3 text-[14.5px] text-[#42574E] placeholder:text-[#7C8B6F] outline-none focus:border-[#5E7268] focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#5E7268]';
const label = 'mb-1.5 block text-[12px] font-semibold uppercase tracking-[0.08em] text-[#5E7268]';
  ...
  if (saved) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-[#FAF8FE] px-5">
        <div className="w-full max-w-[440px] rounded-[20px] border border-[#C6D0C8] bg-white p-8 text-center shadow-[0_12px_40px_-12px_rgba(46,16,80,0.28)]">
          <div className="mx-auto mb-3 grid h-10 w-10 place-items-center rounded-full bg-[#42574E] text-white">✓</div>
          <h2 style={SERIF} className="text-[20px] font-medium text-[#42574E]">Saved — debrief logged</h2>
          <p className="mt-2 text-[13.5px] text-[#4F5F47]">That signal is captured. Schedule the follow-up before it goes cold.</p>
          <div className="mt-5 flex justify-center gap-3">
            <button
              type="button"
              onClick={() => { setV(EMPTY); setSaved(false); }}
              className="rounded-full border border-[#C6D0C8] bg-white px-5 py-2.5 text-[13.5px] font-semibold text-[#42574E] transition hover:border-[#5E7268]"
            >
              Log another
            </button>
            <a href="/company-demo" className="rounded-full bg-[#42574E] px-5 py-2.5 text-[13.5px] font-semibold text-white transition hover:bg-[#42574E]">
              Back to the guide
            </a>
          </div>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#FAF8FE] text-[#42574E] antialiased">
      <div className="mx-auto max-w-[560px] px-5 pb-16 pt-8">
        <div className="mb-1.5 text-[12px] font-semibold uppercase tracking-[0.14em] text-[#5E7268]">Post-call · within ~10 min</div>
        <h1 style={SERIF} className="text-[28px] font-bold tracking-[-0.02em]">Demo debrief</h1>
        <p className="mt-2 text-[14.5px] text-[#4F5F47]">Log it while it’s fresh. Her words, not your summary.</p>

        <div className="mt-6 space-y-5 rounded-[20px] border border-[#C6D0C8] bg-white p-6 shadow-[0_12px_40px_-12px_rgba(46,16,80,0.28)]">
          {error && (
              ...
              {LEAN_IN.map((o) => (
                <button
                  ...
                  className={`rounded-full border px-4 py-2 text-[13px] font-medium transition ${
                    v.leanInMoment === o.v ? 'border-[#42574E] bg-[#42574E] text-white' : 'border-[#C6D0C8] bg-white text-[#4F5F47] hover:border-[#5E7268]'
                  }`}
                >
                  {o.t}
                </button>
          ...
          <div>
            ...
            <label className="mt-2.5 flex cursor-pointer items-start gap-2.5 rounded-[11px] bg-[#E7EDE8] p-3">
              <input
                type="checkbox"
              ...
              />
              <span className="text-[13.5px] text-[#42574E]">
                <b>Asked to search / interrogate the record</b>
                <span className="mt-0.5 block text-[12px] text-[#4F5F47]">Tracked signal — when enough firms ask, that’s the trigger to build it.</span>
              </span>
            </label>
          </div>
                ...
                {OUTCOMES.map((o) => (
                  <button
                    ...
                    className={`rounded-[11px] border px-4 py-2.5 text-left text-[13.5px] font-medium transition ${
                      v.outcome === o.v ? 'border-[#42574E] bg-[#42574E] text-white' : 'border-[#C6D0C8] bg-white text-[#4F5F47] hover:border-[#5E7268]'
                    }`}
                  >
                    {o.t}
                  </button>
          ...
          <button
            ...
            onClick={save}
            disabled={busy}
            className="flex min-h-[48px] w-full items-center justify-center gap-2 rounded-full bg-[#42574E] px-6 py-3 text-[15px] font-semibold text-white transition hover:bg-[#42574E] disabled:opacity-60"
          >
