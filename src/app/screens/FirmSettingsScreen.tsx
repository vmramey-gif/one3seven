        ...
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <div className="flex items-center gap-4 flex-wrap w-full justify-between">
            <div className="flex items-center gap-4 min-w-0">
              ...
              {!setupRequired ? <div className="hidden h-6 w-px bg-[#E7E1FF] sm:block" /> : null}
              <h1 className="truncate text-lg font-semibold text-[#1E1B4B]">
      ...
      <div className="mx-auto max-w-[900px] px-4 py-8 sm:px-6 lg:px-8">
        <div className="space-y-8">
          <motion.section
            ...
            animate={{ opacity: 1, y: 0 }}
            className="rounded-[32px] border border-[#E7E1FF] bg-white/95 p-8 shadow-[0_28px_90px_rgba(31,27,75,0.12)]"
          >
            <h2 className="mb-4 text-lg font-semibold text-[#1E1B4B]">Firm profile</h2>
            {setupRequired ? (
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                <label className="mb-1 block text-xs text-[#1E1B4B]/52">Firm name</label>
                <input
                  value={firmName}
              ...
              <div>
                <label className="mb-1 block text-xs text-[#1E1B4B]/52">Firm code</label>
                {profileComplete && firmCodeDisplay.trim() ? (
                  <>
                    <input
                      value={firmCodeDisplay}
                      readOnly
                      className="w-full rounded-2xl border border-[#DCD3FF] bg-[#F8F6FF] px-4 py-3 font-mono text-sm text-[#1E1B4B]"
                    />
                    ...
                    <div className="mt-3 rounded-2xl border border-[#E7E1FF] bg-[#F7F3FF] px-4 py-3">
                      <p className="mb-1 text-xs font-semibold text-[#1E1B4B]">Your intake link</p>
                      <p className="mb-2 text-xs text-[#1E1B4B]/55 leading-relaxed">
                      ...
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          readOnly
                          className="min-w-0 flex-1 rounded-xl border border-[#DCD3FF] bg-white px-3 py-2 font-mono text-xs text-[#1E1B4B]/70 select-all"
                        ...
                        <button
                          ...
                          className="flex shrink-0 items-center gap-1.5 rounded-xl bg-[#6D4AFF] px-4 py-2 text-xs font-semibold text-white transition hover:bg-[#5B35D5]"
                        >
                  ...
                  <p className="rounded-2xl border border-dashed border-[#DCD3FF] bg-[#F8F6FF] px-4 py-3 text-sm text-[#1E1B4B]/64">
            ...
            <div className="mt-5 rounded-2xl border border-[#E7E1FF] bg-[#F8F6FF] p-5">
              ...
              <div className="grid gap-4 sm:grid-cols-2">
                <div>
                  <label className="mb-1 block text-xs text-[#1E1B4B]/52">State bar</label>
                  <select
                    value={barState}
                    onChange={(e) => setBarState(e.target.value)}
                    className="w-full rounded-2xl border border-[#DCD3FF] bg-white px-4 py-3 text-sm text-[#1E1B4B] focus:border-[#6D4AFF] focus:outline-none focus:ring-4 focus:ring-[#6D4AFF]/10"
                  >
                ...
                <div>
                  <label className="mb-1 block text-xs text-[#1E1B4B]/52">Bar number</label>
                  <input
                    value={barNumber}
                    onChange={(e) => setBarNumber(e.target.value)}
                    placeholder="e.g. 123456"
                    className="w-full rounded-2xl border border-[#DCD3FF] bg-[#F8F6FF] px-4 py-3 font-mono text-sm text-[#1E1B4B] placeholder:text-[#1E1B4B]/38 focus:border-[#6D4AFF] focus:bg-white focus:outline-none focus:ring-4 focus:ring-[#6D4AFF]/10"
                  />
                </div>
              </div>
            </div>
            ...
            {firmProfile?.contact_email ? (
              <div className="mt-4 rounded-2xl border border-[#E7E1FF] bg-[#F8F6FF] px-4 py-3">
                <p className="mb-1 text-xs text-[#1E1B4B]/52">Firm account email</p>
                <p className="text-sm font-medium text-[#1E1B4B]">{firmProfile.contact_email}</p>
              </div>
            ) : null}

            <div className="mt-4 text-sm text-[#1E1B4B]/64">
              <p>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-[#E7E1FF] bg-[#F8F6FF] px-4 py-3">
                <p className="text-xs text-[#1E1B4B]/52">Direct firm-code routing</p>
                <p className="text-sm font-semibold text-[#1E1B4B]">Active</p>
              </div>
              <button
                type="button"
                ...
                className={`rounded-2xl border px-4 py-3 text-left transition-colors ${
                  acceptingCases
                    ...
                    : 'border-[#E7E1FF] bg-[#F8F6FF]'
                }`}
              >
                <p className="text-xs text-[#1E1B4B]/52">Network intake status</p>
                <p className={`text-sm font-semibold ${acceptingCases ? 'text-emerald-700' : 'text-[#1E1B4B]/50'}`}>
                  {acceptingCases ? 'Accepting new intakes' : 'Not accepting'}
                </p>
              </button>
              <div className="rounded-2xl border border-[#E7E1FF] bg-[#F8F6FF] px-4 py-3">
                <p className="text-xs text-[#1E1B4B]/52">Saved filters</p>
          ...
          >
            <div className="mb-6 flex items-start justify-between gap-4 flex-wrap">
              <div>
                <h2 className="text-lg font-semibold text-[#1E1B4B]">Billing &amp; subscription</h2>
                <p className="mt-1 text-sm text-[#1E1B4B]/55">
                  Current plan:{' '}
                  <span className={`font-semibold ${sub.isPaid ? 'text-emerald-600' : 'text-[#6D4AFF]'}`}>
                    {sub.label}
                  </span>
                  {' · '}
                  <span className="text-[#1E1B4B]/50">{sub.subscriptionStatus}</span>
                </p>
              </div>
              {sub.isPaid && (
                <button
                  ...
                  className="shrink-0 rounded-xl border border-[#E7E1FF] bg-white px-4 py-2 text-xs font-semibold text-[#6D4AFF] transition hover:bg-[#F7F3FF] disabled:opacity-60"
                >
                ...
                return (
                  <div
                    ...
                    className={`relative flex flex-col rounded-[22px] border p-5 transition ${
                      isPopular
                        ? 'border-[#6D4AFF] bg-[#F7F3FF] shadow-[0_8px_32px_rgba(109,74,255,0.14)]'
                        : isCurrent
                  ...
                  >
                    {isPopular && (
                      <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[#6D4AFF] px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-white shadow">
                        Most popular
                      </div>
                    )}
                    {isCurrent && (
                    ...
                    <div className="mb-3">
                      <p className="text-[13px] font-bold text-[#1E1B4B]">{plan.label}</p>
                      <div className="mt-1 flex items-end gap-1">
                        <span className="text-[28px] font-black leading-none tracking-tight text-[#1E1B4B]">
                          ${plan.price}
                        </span>
                        <span className="mb-1 text-xs text-[#1E1B4B]/45">/mo</span>
                      </div>
                    </div>

                    <ul className="mb-5 flex-1 space-y-1.5">
                      <li className="flex items-center gap-1.5 text-xs text-[#1E1B4B]/65">
                      ...
                      <button
                        ...
                        className={`rounded-xl py-2.5 text-xs font-semibold transition disabled:opacity-60 ${
                          isPopular
                            ...
                            : 'border border-[#DCD3FF] bg-white text-[#6D4AFF] hover:bg-[#F7F3FF]'
                        }`}
                      >
                        {loading ? 'Opening Stripe…' : 'Start free trial →'}
                      </button>
                    ) : (
                      <div className="rounded-xl border border-dashed border-[#DCD3FF] py-2.5 text-center text-xs text-[#1E1B4B]/40">
            ...
            {/* Enterprise */}
            <div className="mt-4 flex items-center justify-between rounded-[18px] border border-dashed border-[#DCD3FF] bg-[#FAFAFF] px-5 py-4">
              <div>
                <p className="text-[13px] font-bold text-[#1E1B4B]">Enterprise</p>
            ...
            <p className="mt-4 text-center text-[11px] text-[#1E1B4B]/35">
          ...
          <motion.section
            ...
            className="rounded-[32px] border border-[#E7E1FF] bg-white/95 p-8 shadow-[0_28px_90px_rgba(31,27,75,0.12)]"
          >
            <h2 className="mb-2 text-lg font-semibold text-[#1E1B4B]">Team & permissions</h2>
            <p className="mb-4 text-sm text-[#1E1B4B]/64">
              Team access is planned for paid firm workspaces. This beta account currently belongs to the signed-in firm user.
            </p>
            <p className="rounded-2xl border border-dashed border-[#DCD3FF] bg-[#F8F6FF] px-4 py-3 text-xs text-[#1E1B4B]/52">
          ...
          <motion.section
            ...
            className="rounded-[32px] border border-[#E7E1FF] bg-white/95 p-8 shadow-[0_28px_90px_rgba(31,27,75,0.12)]"
          >
            <h2 className="mb-2 text-lg font-semibold text-[#1E1B4B]">Account security</h2>
            <p className="mb-3 text-sm text-[#1E1B4B]/64">
          ...
          >
            <div className="mb-4 flex items-center gap-3">
              <Filter className="h-5 w-5 text-[#6D4AFF]" />
              <h2 className="text-lg font-semibold text-[#1E1B4B]">Intake Routing Preferences</h2>
            </div>
            <p className="mb-6 text-sm leading-relaxed text-[#1E1B4B]/64">
            ...
            <div className="space-y-6">
              <div>
                <div className="mb-3 flex items-center gap-2">
                  <MapPin className="h-4 w-4 text-[#6D4AFF]" />
                  <div className="text-sm font-medium text-[#1E1B4B]">Geography Preferences</div>
                </div>
              ...
              <div>
                <div className="mb-3 text-sm font-medium text-[#1E1B4B]">
              ...
              <div>
                <div className="mb-3 text-sm font-medium text-[#1E1B4B]">Complete Packet Signals</div>
                <div className="grid gap-2 sm:grid-cols-2">
                  {REQUIRED_PACKET_SIGNALS.map((signal) => (
                    <div key={signal} className="rounded-2xl border border-[#E7E1FF] bg-[#F8F6FF] px-4 py-3">
                      <p className="text-sm font-medium text-[#1E1B4B]">{signal}</p>
                      <p className="mt-1 text-xs text-[#1E1B4B]/52">Review-preparation signal, not a legal requirement.</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <motion.section
            ...
            className="rounded-[32px] border border-[#E7E1FF] bg-[#F8F6FF] p-8 shadow-[0_18px_56px_rgba(31,27,75,0.09)]"
          >
            <div className="flex items-start gap-3">
              <Info className="mt-0.5 h-5 w-5 flex-shrink-0 text-[#6D4AFF]" />
              <div>
                ...
                <p className="mt-3 text-xs leading-relaxed text-[#1E1B4B]/64">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
            <div className="min-w-0 flex-1 space-y-2">
              {settingsError ? (
                ...
                <p className="text-sm text-[#1E1B4B]/52">
              ...
              {setupRequired ? (
                <div className="flex flex-col gap-2">
                  <button
                    ...
                    className="w-fit text-xs text-[#1E1B4B]/60 underline underline-offset-2 hover:text-[#1E1B4B] disabled:opacity-50"
                  >
                    {diagBusy ? 'Running DB diagnostics…' : 'Run DB diagnostics (temp)'}
                  </button>
                  {diagSummary ? (
                    <p className="break-words font-mono text-xs text-[#1E1B4B]/60">{diagSummary}</p>
                  ) : null}
                </div>
              ) : null}
            </div>
            <button
              ...
              disabled={isSaving}
              className="flex shrink-0 items-center justify-center gap-2 rounded-full bg-[#6D4AFF] px-8 py-3 font-medium text-white shadow-[0_18px_48px_rgba(109,74,255,0.26)] transition hover:-translate-y-0.5 hover:bg-[#5B35D5] disabled:translate-y-0 disabled:opacity-60"
            >
              <Save className="w-4 h-4" />
  ...
  return (
    <button
      ...
      className={`px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
        selected
          ? 'bg-[#6D4AFF] text-white shadow-[0_10px_24px_rgba(109,74,255,0.18)]'
          : 'border border-[#DCD3FF] bg-white text-[#1E1B4B]/70 hover:border-[#B8A8FF] hover:bg-[#F7F3FF] hover:text-[#1E1B4B]'
      }`}
    >
      {children}
    </button>
