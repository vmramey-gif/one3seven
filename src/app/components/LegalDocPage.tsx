        ...
        <div className="mt-10 space-y-9">
          {doc.sections.map((section) => (
            <section key={section.heading}>
              <h2 className="mb-3 text-[18px] font-bold tracking-tight text-[#1E1B4B]">{section.heading}</h2>
              <div className="space-y-3">
                {section.blocks.map((block, i) => {
                  if (block.type === 'subhead') {
                    return (
                      <h3 key={i} className="pt-1 text-[14px] font-semibold text-[#1E1B4B]">{block.text}</h3>
                    );
                  }
                    ...
                    return (
                      <ul key={i} className="space-y-1.5">
                        {block.items.map((item, j) => (
                          <li key={j} className="flex gap-2.5 text-[14.5px] leading-relaxed text-[#1E1B4B]/72">
                  ...
                  return (
                    <p key={i} className="text-[14.5px] leading-relaxed text-[#1E1B4B]/72">{block.text}</p>
                  );
                })}
              </div>
            </section>
