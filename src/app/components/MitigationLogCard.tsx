 ...
 * theirs. English v1; ES localization is a fast-follow.
 */

const SERIF = { fontFamily: "'Fraunces', Georgia, serif" } as const;
const MONO = { fontFamily: '"IBM Plex Mono", ui-monospace, Menlo, monospace' } as const;

const OUTCOME_LABELS: Record<MitigationOutcome, string> = {
  applied: 'Applied',
  interview: 'Interview',
  offer: 'Offer',
  hired: 'Hired',
...
const OUTCOMES = Object.keys(OUTCOME_LABELS) as MitigationOutcome[];
...
export function MitigationLogCard({ intakeId }: { intakeId: string | null }) {
  ...
  return (
    <section style={BODY} className="rounded-[22px] border border-[#CBD6CF] bg-white/80 p-6 sm:p-7">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div style={MONO} className="mb-1 flex items-center gap-2 text-[11px] uppercase tracking-[0.14em] text-[#42574E]">
            <Briefcase className="h-3.5 w-3.5" /> Your job-search record
          </div>
          <h3 style={SERIF} className="text-[20px] font-semibold tracking-[-0.01em] text-[#17181C]">
            Looking for work? Keep a note of it here.
          </h3>
          <p className="mt-1.5 max-w-[52ch] text-[13.5px] leading-relaxed text-[#5a5f58]">
            Jot down where you applied and what happened. It becomes part of your record — and, like everything here, it stays yours.
          </p>
        </div>
        {!showForm ? (
          <button
        ...
        <div className="mt-4 rounded-[16px] border border-[#E4E5DE] bg-[#FBFBFA] p-4">
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="block">
              <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-[#6a6d66]">Date you applied</span>
              <input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className={inputCls} />
            </label>
            <label className="block">
              <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-[#6a6d66]">What happened</span>
            ...
            <label className="block">
              <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-[#6a6d66]">Employer / company</span>
              <input value={form.employer} onChange={(e) => setForm({ ...form, employer: e.target.value })} placeholder="e.g. Acme Foods" className={inputCls} />
            </label>
            <label className="block">
              <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-[#6a6d66]">Role / position</span>
              <input value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} placeholder="e.g. Warehouse associate" className={inputCls} />
            </label>
            <label className="block sm:col-span-2">
              <span className="mb-1 block text-[11px] font-semibold uppercase tracking-wide text-[#6a6d66]">Notes (optional)</span>
              <input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Anything you want to remember about it" className={inputCls} />
            </label>
          </div>
      ...
      <div className="mt-4">
        {loading ? (
          <p className="text-[13px] text-[#8a938c]">Loading your record…</p>
        ) : entries.length === 0 ? (
          <p className="rounded-[14px] border border-dashed border-[#CBD6CF] bg-[#FBFBFA] px-4 py-5 text-center text-[13px] text-[#6a6d66]">
            Nothing logged yet. Add the first place you applied whenever you’re ready — there’s no wrong time to start.
          </p>
        ) : (
          <ul className="flex flex-col gap-2">
            {entries.map((e) => (
              <li key={e.id} className="flex items-start gap-3 rounded-[14px] border border-[#E4E5DE] bg-[#FBFBFA] px-4 py-3">
                ...
                <button type="button" onClick={() => void persist(entries.filter((x) => x.id !== e.id))} aria-label="Remove entry" className="mt-0.5 flex-none text-[#b6bbb2] transition hover:text-[#A8512B]">
      ...
      <p className="mt-4 border-t border-[#EEF0EA] pt-3 text-[11.5px] leading-relaxed text-[#8a8f88]">
        This is your own record. one3seven never scores it or decides whether it’s “enough” — that’s a question for you and your attorney.{saving ? ' · Saving…' : ''}
      </p>
    </section>
  );
}
