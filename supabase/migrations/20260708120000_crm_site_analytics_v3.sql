-- Growth analytics v3: separate INTERNAL (founder/rep) traffic from real prospect
-- traffic. Internal = /hq, /one3sevenhq, /company-demo*. Those paths were inflating
-- total_sessions, avg_session_seconds, and the daily visits trend (reps sitting in
-- the CRM generate pageviews + heartbeats). Now excluded from the public numbers and
-- surfaced separately as internal_sessions / internal_visits. Apply after v2.
--
-- Internal-path predicate used throughout: path in ('/hq','/one3sevenhq') or path like '/company-demo%'
...
begin
  ...
  select json_build_object(
    ...
    -- Public sessions: exclude sessions whose only activity is internal (HQ/company-demo).
    'total_sessions',   (
      select count(distinct session_id) from public.web_events
      where not (path in ('/hq','/one3sevenhq') or path like '/company-demo%')
    ),
    'avg_session_seconds', (
    ...
    -- Internal (founder/rep) activity, kept separate so it never pollutes the numbers above.
    'internal_sessions', (
      select count(distinct session_id) from public.web_events
      where path in ('/hq','/one3sevenhq') or path like '/company-demo%'
    ),
    'internal_visits', (
      select count(*) from public.web_events
      where event = 'pageview' and (path in ('/hq','/one3sevenhq') or path like '/company-demo%')
    ),
    ...
    'daily', (
      select coalesce(json_agg(row_to_json(d) order by d.day_sort), '[]'::json) from (
        select
          ...
          (select count(*) from public.web_events w
             where w.event = 'pageview' and w.created_at::date = gs::date
               and not (w.path in ('/hq','/one3sevenhq') or w.path like '/company-demo%')) as visits,
