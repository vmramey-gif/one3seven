-- Persistent notifications for document-request loop (firm ↔ worker).
-- Types: firm_document_request, worker_documents_submitted

create table if not exists public.notifications (
  id uuid primary key default gen_random_uuid(),
  recipient_user_id uuid not null references auth.users (id) on delete cascade,
  recipient_kind text not null check (recipient_kind in ('worker', 'firm')),
  notification_type text not null check (
    notification_type in ('firm_document_request', 'worker_documents_submitted')
  ),
  title text not null,
  body text,
  payload jsonb not null default '{}'::jsonb,
  read_at timestamptz,
  related_intake_id uuid references public.intakes (id) on delete set null,
  related_route_id uuid references public.firm_intake_routes (id) on delete set null,
...
-- Security definer insert helper (callable from other definer RPCs only; not granted to authenticated).
...
begin
  ...
  if p_notification_type not in ('firm_document_request', 'worker_documents_submitted') then
...
-- Patch: firm document request creates a persistent worker notification.
